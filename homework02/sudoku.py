...
def read_sudoku(filename: str) -> List[List[str]]:
    ...
    with open(filename) as f:
        content = f.read()
    digits = [с for c in content if c in '123456789.']
    grid = group(digits, 9)
    return grid
...
def check_solution(solution: List[List[str]]) -> bool:
    ...
    set_values = set('123456789')
    for row in range(len(solution)):
        row_values = set(get_row(solution, (row, 0)))
        if row_values.intersection(set_values) != set('123456789'):
            return False

    for col in range(len(solution)):
        col_values = set(get_col(solution, (row, 0)))
        if col_values.intersection(set_values) != set('123456789'):
            return False

    for block in range(len(solution)):
        block_values = set(get_block(solution, (row, 0)))
        if block_values.intersection(set_values) != set('123456789'):
            return False
